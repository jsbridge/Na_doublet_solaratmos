;+
;
; Models the opacity of the NaI D2 lines (lambda = 5890)
; Question: when doing two lines, do you sum the opacities?
;
;-
function sodium_D2, lambda, temp, Pgas, Pe, vel
...
lam_core = 5890d   ; angstroms
e = 4.803242d-10   ; cgs
c = 2.99792458d10  ; cm/s
k = 1.3806503d-16  ; ergs/K
g_l = 2d   ; for other line, 2d
g_u = 4d   ; for other line, 2d
A_ul = 2.46d8/g_u; for other line, 1.23d8/g_u

x = where(!matchabund.name eq 'Na')
Q = (!matchabund.abund)[x]

gam4 = 10^(19d + ((-15.17d * 2d)/3d) + alog10(Pe) - ((5d * alog10(temp))/6d))

y = where(!ioniztn.name eq 'Na')
chi = 0;2.10499442   ; eV, for other line, 2.1028523
C6 = 0.3d-30 * ((1d/(!ioniztn.ion1[y] - chi - chi_lam)^2d) - (1d/(!ioniztn.ion1[y] - chi)^2d))
gam6 = 10^(20d + 0.4d * alog10(C6) + alog10(Pgas) - 0.7d * alog10(temp))
